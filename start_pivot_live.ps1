# forex_cpr_ibkr - PIVOT strategy LIVE launch (Windows / PowerShell)
#
# Places REAL CFD orders on the configured account using the NEW pivot/SR +
# quality-SuperTrend strategy (runner_pivot.py). The old CPR strategy is
# unaffected - use start_live.ps1 for that one.
#
# Usage:
#     .\start_pivot_live.ps1
#     .\start_pivot_live.ps1 -LotSize 0.01          # smaller size for break-in week
#     .\start_pivot_live.ps1 -ForceCleanRestart     # wipe state file (dangerous)
#
# If execution policy blocks scripts:
#     powershell -ExecutionPolicy Bypass -File .\start_pivot_live.ps1
#
# What it does:
#   - Sets LOT_SIZE + CFD_ACCOUNT env vars for this process
#   - Asks for explicit confirmation before placing real orders
#   - Preserves pivot_strategy_state.json across restarts (state-based recovery)
#   - Streams logs to terminal AND appends to pivot_live.log
...
Write-Host ' forex_cpr_ibkr - PIVOT strategy LIVE MODE (real orders)' -ForegroundColor Red
Write-Host '========================================================' -ForegroundColor Red
Write-Host " account      : $Account"
Write-Host " lot size     : $LotSize lot ($units units per order)"
Write-Host " port         : 4001 (live IB Gateway)"
Write-Host " state file   : pivot_strategy_state.json (preserved across restarts)"
Write-Host " log file     : pivot_live.log (appended)"
Write-Host '========================================================' -ForegroundColor Red
Write-Host ''
...
# Build argument list
$pythonArgs = @('runner_pivot.py', '--live', '--i-really-mean-it')
if ($ForceCleanRestart) {
    $pythonArgs += '--force-clean-restart'
    Write-Host 'WARNING: --force-clean-restart will WIPE pivot_strategy_state.json' -ForegroundColor Yellow
...
python @pythonArgs 2>&1 | Tee-Object -FilePath pivot_live.log -Append
